...
def contributors(ack, say, command):
    ...
    prs, issues, comments = fetch_github_data(
        GITHUB_REPOSITORY_OWNER, GITHUB_REPOSITORY_NAME, since_date
    )
...
def handle_reminder(ack, body, say):
    ...
    try:
        ...
        if not channel_id or not message or not when:
            say(text="Invalid command format. Please use [channel_id] [what] [when].")
            logging.error("Invalid command format")
            return

        post_at = convert_to_datetime(when)
        if not post_at:
            say(text=f"Invalid time format for 'when': {when}.")
...
def schedule_message(channel_id, text, post_at):
    try:
        response = app.client.chat_scheduleMessage(
            channel=channel_id, text=text, post_at=int(post_at.timestamp())
        )
        return response
    except SlackApiError as e:
